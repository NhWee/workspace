...
<body>
  ...
  <section class="panel">
    ...
    <div class="grid">
      ...
      <label><span>Foam <output id="foamValue"></output></span><input id="foam" type="range" min="0" max="2.2" step="0.01" value="1.0"></label>
  ...
  const crest = new THREE.Color(0xaee8f5);
  ...
  function sampleOcean(x, z, t) {
    ...
    for (const w of waves) {
      ...
      foam += smoothstep(0.70, 0.98, sn) * w.q * amp * w.k * state.foam;
    }
    ...
    return { x: x + dx, y, z: z + dz, foam: clamp(foam, 0, 1), slope };
  }
  ...
  function updateOcean(t, dt) {
    ...
    for (let i = 0; i < pos.count; i++) {
      ...
      const c = deep.clone().lerp(mid, h).lerp(crest, smoothstep(0.72, 1.0, h) * 0.36).lerp(foamColor, state.showFoam ? foamAttr[i] * 0.72 : 0);
      colors[i * 3 + 0] = c.r;
      colors[i * 3 + 1] = c.g;
      colors[i * 3 + 2] = c.b;
    }
    ...
    if (state.showSpray && state.spray > 0 && frameIndex % 2 === 0) {
      for (let k = 0; k < 10 * state.spray; k++) {
        const x = (Math.random() - 0.5) * size * 0.82;
        const z = (Math.random() - 0.5) * size * 0.82;
        const s = sampleOcean(x, z, t);
        if (s.foam > 0.70 && spray.length < maxSpray && Math.random() < s.foam * 0.20) {
          spray.push({
            x: s.x, y: s.y + 0.35, z: s.z,
            vx: (Math.random() - 0.5) * 0.25,
            vy: 1.0 + Math.random() * 2.4 * state.spray,
  ...
  document.getElementById('storm').onclick = () => {
    setControl('wind', 2.05); setControl('chop', 1.72); setControl('swell', 1.55); setControl('foam', 1.72); setControl('spray', 1.45); setControl('sun', 0.62);
  };
  document.getElementById('calm').onclick = () => {
    setControl('wind', 0.65); setControl('chop', 0.62); setControl('swell', 0.58); setControl('foam', 0.42); setControl('spray', 0.20); setControl('sun', 0.90);
  };
  ...
  function animate(now) {
    ...
    waterMat.roughness = 0.40 - 0.18 * state.sun;
