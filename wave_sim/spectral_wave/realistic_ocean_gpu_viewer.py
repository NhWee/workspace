...
<body>
  ...
  const crestAttr = new Float32Array(pos.count);
  ...
  const crestFoamColor = new THREE.Color(0xd9eff0);
  ...
  function sampleOcean(x, z, t) {
    ...
    return { x: x + dx, y, z: z + dz, foam, breaking: crestBreak, breakup, slope, slopeX, slopeZ };
  }
  ...
  function updateOcean(t, dt) {
    ...
    for (let i = 0; i < pos.count; i++) {
      ...
      crestAttr[i] = lerp(crestAttr[i], s.breaking, 0.14);
      foamSum += foamAttr[i];
      ...
      const crestRibbon = smoothstep(0.62, 0.90, crestAttr[i]) * (0.32 + 0.68 * s.breakup);
      const foamBlend = state.showFoam ? clamp(foamAttr[i] * 0.48, 0, 0.50) : 0;
      const c = deep.clone()
        .lerp(mid, h)
        .lerp(crest, smoothstep(0.72, 1.0, h) * 0.25)
        .lerp(reflectionColor, clamp(reflection, 0, 0.42))
        .lerp(sunGlintColor, clamp(sparkle, 0, 0.68))
        .lerp(foamColor, foamBlend)
        .lerp(crestFoamColor, state.showFoam ? crestRibbon * 0.18 : 0);
      colors[i * 3 + 0] = c.r;
      colors[i * 3 + 1] = c.g;
      colors[i * 3 + 2] = c.b;
    }
